...
try:
    ...
    assert False, dataClass.install_suggestion

import AppKit

if sys.platform !=  dataClass.OSXPlatform:
    raise Exception(dataClass.OSXPlatform_ONLY)

keyboardMapping = dict([(key, None) for key in dataClass.KEY_NAMES])
keyboardMapping.update(dataClass.keyboardmapping_update)
'''({
    'a': 0x00, # kVK_ANSI_A
    's': 0x01, # kVK_ANSI_S
    'd': 0x02, # kVK_ANSI_D
...
def _v_scroll(clicks, x=None, y=None):
    _move_to(x, y)
    clicks = int(clicks)
...
def _h_scroll(clicks, x=None, y=None):
    _move_to(x, y)
    clicks = int(clicks)
    for _ in range(abs(clicks) // 10):
        scroll_wheel_event = Quartz.CGEventCreateScrollWheelEvent(
...
def _multi_click(x, y, button, num):
    btn = None
    down = None
    up = None
