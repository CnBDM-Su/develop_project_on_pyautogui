...
try:
    ...
    assert False, config.install_suggestion
...
class Mouse_OS:
    def __init__(self):
        if sys.platform !=  config.OSXPlatform:
            raise Exception(config.OSXPlatform_ONLY)

    def _scroll(self, clicks, ish=False, x=None, y=None):
        self._move_to(x, y)
        clicks = int(clicks)
        if(ish):
        ...
        if (ish):
            ...
            scroll_wheel_event = Quartz.CGEventCreateScrollWheelEvent(
                ...
                Quartz.kCGScrollEventUnitLine,  # units
                1,  # wheelCount (number of dimensions)
                10 if clicks >= 0 else -1 * (-clicks % 10))  # vertical movement

        Quartz.CGEventPost(Quartz.kCGHIDEventTap, scroll_wheel_event)

    '''def _v_scroll(self, clicks, x=None, y=None):
        self._move_to(x, y)
        clicks = int(clicks)
        for _ in range(abs(clicks) // 10):
            scroll_wheel_event = Quartz.CGEventCreateScrollWheelEvent(
    ...
    def _h_scroll(self, clicks, x=None, y=None):
        self._move_to(x, y)
        clicks = int(clicks)
        for _ in range(abs(clicks) // 10):
            scroll_wheel_event = Quartz.CGEventCreateScrollWheelEvent(
    ...
    def _mouse_event(self, x, y, button, behavior):
        if(behavior == 'up'):
        ...
        if button == config.LEFT:
            self._send_mouse_event(event, x, y, Quartz.kCGMouseButtonLeft)
        elif button == config.MIDDLE:
            self._send_mouse_event(event, x, y, Quartz.kCGMouseButtonCenter)
        elif button == config.RIGHT:
            self._send_mouse_event(event, x, y, Quartz.kCGMouseButtonRight)
        else:
            assert False, config.button_argument
    ...
    def _multi_click(self, x, y, button, num):
        if button == config.LEFT:
    ...
    def _drag_to(self, x, y, button):
        if button == config.LEFT:
            self._send_mouse_event(Quartz.kCGEventLeftMouseDragged, x, y, Quartz.kCGMouseButtonLeft)
        elif button == config.MIDDLE:
            self._send_mouse_event(Quartz.kCGEventOtherMouseDragged, x, y, Quartz.kCGMouseButtonCenter)
        elif button == config.RIGHT:
            self._send_mouse_event(Quartz.kCGEventRightMouseDragged, x, y, Quartz.kCGMouseButtonRight)
        else:
            assert False, config.button_argument
        time.sleep(0.01)  # needed to allow OS time to catch up.

    def _send_mouse_event(self, ev, x, y, button):
        mouse_event = Quartz.CGEventCreateMouseEvent(None, ev, (x, y), button)
        Quartz.CGEventPost(Quartz.kCGHIDEventTap, mouse_event)

    def _move_to(self, x, y):
        self._send_mouse_event(Quartz.kCGEventMouseMoved, x, y, 0)
        time.sleep(0.01)  # needed to allow OS time to catch up.
    ...
    @property
    def _size(self):
        return Quartz.CGDisplayPixelsWide(Quartz.CGMainDisplayID()), Quartz.CGDisplayPixelsHigh(
            Quartz.CGMainDisplayID())
    '''def _scroll(self, clicks, x=None, y=None):
        self._v_scroll(clicks, x, y)'''


'''keyboardMapping = dict([(key, None) for key in config.KEY_NAMES])
keyboardMapping.update(config.keyboardmapping_update)
({
    'a': 0x00, # kVK_ANSI_A
    's': 0x01, # kVK_ANSI_S
    'd': 0x02, # kVK_ANSI_D
